---
title: "Code for creating an expression matrix"
...
**#this is the code for the analysis of singel cell sequencing in the manuscript entitled "A genomic portrait of zebrafish transposable elements and their spatiotemporal embryonic expression"**
<br>
##re-align reads to dr11 (all file downloaded from Farrell et al., are .bam files)
...
#create a reference file for TE and gene annotation #grep non-readthrough loci from Quirzes gtf file
```{bash, eval=FALSE}
...
#filter cells in R, to get rid of cells with abnormal (too low/high) transcipt numbers
```{bash, eval=FALSE}
library(Seurat)
library(dplyr)
library(data.table)
...
#Jon helped me to merge all samples into one big matrix
#named filtered_clean_new_annot2.txt
